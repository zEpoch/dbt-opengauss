...
from dbt.adapters.opengauss import openGaussConnectionManager
from dbt.adapters.opengauss import openGaussColumn
from dbt.adapters.opengauss import openGaussRelation
...
GET_RELATIONS_MACRO_NAME = "openGauss_get_relations"


@dataclass
class openGaussIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["openGaussIndexConfig"]:
...
@dataclass
class openGaussConfig(AdapterConfig):
    unlogged: Optional[bool] = None
    indexes: Optional[List[openGaussIndexConfig]] = None


class openGaussAdapter(SQLAdapter):
    Relation = openGaussRelation
    ConnectionManager = openGaussConnectionManager
    Column = openGaussColumn

    AdapterSpecificConfigs = openGaussConfig
    ...
    @available
    def parse_index(self, raw_index: Any) -> Optional[openGaussIndexConfig]:
        return openGaussIndexConfig.parse(raw_index)
    ...
    def _get_catalog_schemas(self, manifest):
        # openGauss only allow one database (the main one)
