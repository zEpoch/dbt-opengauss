...
logger = AdapterLogger("openGauss")


@dataclass
class openGaussCredentials(Credentials):
    host: str
    user: str
    port: Port
    password: str  # on openGauss the password is mandatory
...
class openGaussConnectionManager(SQLConnectionManager):
    TYPE = "openGauss"
    ...
    def exception_handler(self, sql):
        try:
            ...
            logger.debug("openGauss error: {}".format(str(e)))
    ...
    def open(cls, connection):
        ...
        kwargs = {}
        # we don't want to pass 0 along to connect() as openGauss will try to
        ...
        if search_path is not None and search_path != "":
            # see https://openGaussql.org/docs/9.5/libpq-connect.html
        ...
        try:
            ...
            logger.debug(
                "Got an error when attempting to open a openGauss " "connection: '{}'".format(e)
            )
